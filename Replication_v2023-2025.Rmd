...
dengue_incidence_map_themed <- ggplot() +
  ...
  geom_sf(data = dengue_capitals_map_data, aes(shape = incidence_cat), 
         fill = NA, color = "darkgrey", alpha = 0.8, size = 4, stroke = 2) +
   scale_fill_brewer(
    palette = "Blues",
...
risk_analysis <- function(df, analysis_year = 2024) {
  ...
  risk_final <- tibble(epi_week = 1:53) %>% # Ensure all epi_weeks are present
  ...
  risk_final <- risk_final %>%
    ...
    riskassessment = case_when(
      ...
      # EMERGENCY - Condition 5 AND/OR Condition 6
      ...
      (increased_incidence_high == 1 & increased_deaths_4w == 1 & lethality == 1) |
      (increased_incidence_high == 1 & increased_deaths_4w == 1) |
     (increased_incidence_high == 1 & lethality == 1) |
      (increased_deaths_4w == 1 & lethality == 1) |
     (increased_incidence_high == 1 | increased_deaths_4w == 1 | lethality == 1) ~ 2L,
...
emergency_state <- tibble::tribble(
    ...
    "ES", "21/02/2024",
    "RJ", "21/02/2024",
    "SC", "22/02/2024", # Same day as Florianopolis
    "MG", "26/02/2024",
    "AP", "28/02/2024",
    "SP", "05/03/2024",
    "RS", "12/03/2024",
...
# Breaks
dengue_week_breaks <- c(1, 5, 13, Inf)
...
lag_map_data_states <- states_sf %>%
  ...
  mutate(
    ...
    lag_category = case_when(
      is.na(first_week_alert_2024) ~ "No Alert Triggered",
      is.na(declaration_week) ~ "No Declaration",
      lag_weeks <= 0 ~ "Declared On or Before Alert",
      lag_weeks > 0 & lag_weeks <= 4 ~ "1-4 Weeks After Alert",
      lag_weeks > 4 ~ "5+ Weeks After Alert"
    ),
    
    # Convert to a factor to control the order in the plot legend
    lag_category = factor(lag_category, levels = c(
      "Declared On or Before Alert", "1-4 Weeks After Alert", 
      "5+ Weeks After Alert", "No Declaration", "No Alert Triggered"
    ))
  )
...
lag_map <- ggplot() +
  ...
  scale_fill_brewer(
    palette = "YlOrRd",
    name = "State Response Lag",
    drop = T, 
    na.translate = F
  ) +
  scale_shape_manual(
    name = "Capital Response Lag",
            values = c(21, 25),
    drop = T, 
    na.translate = F
  ) +
...
```{r create-map-panel, fig.width=14, fig.height=12}
map_panel <- (dengue_incidence_map_themed | dengue_deaths_rate_map_themed) / (dengue_risk_map_themed | lag_map) 

print(map_panel)

ggsave(here("Graphs", "Map_Panel.png"), map_panel, width = 18, height = 14,dpi = 300)
```

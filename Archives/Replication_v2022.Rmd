...
dengue_incidence_map_themed <- ggplot() +
  ...
  geom_sf(data = dengue_capitals_map_data, aes(shape = incidence_cat), 
   ...
   scale_fill_brewer(
    palette = "Blues",
    name = "Cumulative Dengue Incidence (States)\n(per 100,000 pop.)",
    na.translate = FALSE,
    drop = TRUE 
  ) +
  scale_shape_discrete(
    name = "Cumulative Dengue Incidence (Capitals)\n(per 100,000 pop.)",
    drop = FALSE
  ) +
  theme_bw() +
  theme(
...
dengue_deaths_rate_map_themed <- ggplot() +
  ...
  scale_fill_brewer(
    palette = "Reds",
    name = "Cumulative Dengue Death Rate (States)\n(per 100,000 pop.)",
    na.translate = FALSE,
    drop = TRUE
  ) +
  scale_shape_discrete(
    name = "Cumulative Dengue Death Rate (Capitals)\n(per 100,000 pop.)",
    drop = FALSE
  ) +
  theme_bw() +
  theme(
...
create_risk_plot <- function(data, facet_var = NULL) {
  ...
  p <- data %>%
    ...
    # Endemic channel 
    geom_ribbon(aes(ymin = low, ymax = high, fill = "Endemic Channel", group = 1), alpha = 0.5) +
    ...
    # Scale x-axis
    scale_x_continuous(breaks = seq(min(data$epi_week), max(data$epi_week), by=10)) +

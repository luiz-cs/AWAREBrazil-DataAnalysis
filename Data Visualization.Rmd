...
cores2 <- colorRampPalette(c("mistyrose", "orange", "red", "darkred"))
cores_lancet <- colorRampPalette(pal_lancet("lanonc")(5))
...
### colocando mapa com valores por Estado

dados_estados <- arboviruses_base %>% 
  filter(year == 2024) %>%
  mutate(code_state = substr(as.character(ibge), 1, 2)) %>%  # extrai os dois primeiros dígitos do código IBGE
  group_by(code_state) %>%
  summarise(
    incidence = sum(incidence, na.rm = TRUE),
    cases = sum(cases, na.rm = TRUE),
    deaths = sum(deaths, na.rm = TRUE),
    deaths_per_capta = sum(deaths, na.rm = TRUE) / sum(population, na.rm = TRUE),
    deaths_per_case = sum(deaths, na.rm = TRUE) / sum(cases, na.rm = TRUE)
  ) %>%
  mutate(code_state = as.integer(code_state))

mapa_estados_dados <- left_join(estados, dados_estados, by = "code_state")

g1_estado <- ggplot(data = mapa_estados_dados) +
  geom_sf(aes(fill = incidence), color = "white") +
  scale_fill_gradientn(
    colours = cores(200),
    na.value = "gray90",
  ...
  labs(
    title = "Cumulative Incidence by State - 2024",
    subtitle = ""
  ) +
  theme_minimal() +
  theme(
...
ggsave(here("G1 - Incidence Brasil - state visualization- 2024.png"), g1_estado, width = 10, height = 10)
...
g3_estado <- ggplot(data = mapa_estados_dados) +
  geom_sf(aes(fill = deaths_per_capta*1000), color = "white") +
  scale_fill_gradientn(
    colours = cores2(200),
    na.value = "gray90",
    name = "Cumulative deaths per 1.000 pop."
  ) +
  labs(
    title = "Cumulative deaths per 1.000 pop. by State - 2024",
    subtitle = ""
  ) +
  theme_minimal() +
  theme(
...
ggsave(here("G3 - Deaths per capta - Brasil - state visualization- 2024.png"), g3_estado, width = 10, height = 10)

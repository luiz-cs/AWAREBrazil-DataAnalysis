...
arboviruses_state_ind <- arboviruses_base %>%
  ...
  summarise(
    ...
    # CORRECT LOGIC: Sum the populations of all municipalities within the state
...
create_arbovirus_plot(
  ...
  facet_by = "muni_name",
  year_interval = 2,
  file_name = "Incidence of All Dengue Selected Municipalities.png"
)
```
...
dengue_incidence_map_themed <- ggplot() +
  geom_sf(data = dengue_states_map_data, aes(fill = incidence_cat), color = "black", linewidth = 0.2) +
...
ggsave(here("Graphs", paste0("Map Dengue Incidence 2024", epi_year_to_plot, ".png")), dengue_incidence_map_themed, width = 12, height = 10, dpi = 300)

### MAP 2: CUMULATIVE DEATH RATE

dengue_deaths_rate_map_themed <- ggplot() +
  geom_sf(data = dengue_states_map_data, aes(fill = death_rate_cat), color = "black", linewidth = 0.2) +
...
ggsave(here("Graphs", paste0("Map Dengue Death 2024", epi_year_to_plot, ".png")), dengue_deaths_rate_map_themed, width = 12, height = 10, dpi = 300)
```
...
risk_analysis <- function(df, analysis_year = 2024) {
  ...
  tryCatch({
    ...
    preliminary_channel <- df %>%
      ...
      # 3. For each week, calculate the historical median and standard deviation of incidence.
      summarise(median_incidence = median(incidence, na.rm = TRUE), sd_incidence = sd(incidence, na.rm = TRUE), .groups = "drop") %>%
      # 4. Calculate the upper alert threshold (95% confidence interval).
      mutate(high = median_incidence + (1.96 * sd_incidence))
    ...
    control_data <- df %>%
      ...
      # 2. Group by week.
      group_by(week2 = epidemiologic_week_startsweek40) %>%
      # 3. Recalculate the median and standard deviation using this cleaner data.
      summarise(median_incidence = median(incidence, na.rm = TRUE), sd_incidence = sd(incidence, na.rm = TRUE), .groups = "drop") %>%
      # 4. Create the final upper and lower thresholds for the control diagram.
      mutate(high = median_incidence + 1.96*sd_incidence, low = pmax(0, median_incidence - 1.96*sd_incidence)) %>%
...
create_risk_plot <- function(data, title, subtitle, facet_var = NULL) {
  ...
  p <- data %>%
    ggplot(aes(x = week2)) +
    # Endemic channel rectangle - background
    geom_rect(aes(xmin = week2 - 0.5, xmax = week2 + 0.5, ymin = low, ymax = high, fill = "Endemic Channel"), alpha = 0.5) +
...
dengue_risk_country <- create_risk_plot(
  ...
  subtitle = ("Solid line = Creation of Public Health Emergency Operations Center. Dashed lines = Emergency Declarations by States.")) +
...
dengue_risk_state <- create_risk_plot(
  ...
  subtitle = "Dashed lines = Emergency Declarations by States",
  facet_var = "uf"
) + geom_vline(data = emergency_state, aes(xintercept = week2), linetype = "dashed", color = "black")
...
dengue_risk_map_themed <- ggplot() +
 geom_sf(data = dengue_map_data_states, aes(fill = alert_week_group), color = "black", linewidth = 0.2) +
...
ggsave(here("Graphs", paste0("Map First Week Alert 2024", epi_year_to_plot, ".png")), dengue_risk_map_themed, width = 12, height = 10, dpi = 300)
```